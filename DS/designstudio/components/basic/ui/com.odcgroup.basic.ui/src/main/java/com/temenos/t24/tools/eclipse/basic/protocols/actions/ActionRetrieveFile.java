...
/**
 * Returns the contents of a program stored in the server. It CHECKS for locks
 * and will LOCK the file in the server. 
 */
public class ActionRetrieveFile extends AbstractAction {
    ...
    public Response processAction(String[] args) {
        ...
        String serverDirectory = args[2]; // remote server directory where
                                            // file will be saved
        String localUsername = args[3]; // local username, used for locking the
                                        // file
        initialiseResponse();
        ...
        if (!resProcess.getPassed()) {
            // This point is reached if the program was locked by a different
            // user, or the comms failed while performing the check,
            // either way return the response
            return resProcess;
        }
        
        // If it is not locked at all, then lock it, so no other user can open
        // and modify it.
        if (!ac.isLocked(resProcess)) {
            resProcess = ac.lockFile(sessionToken, basicFilenameNoPrefix);
            if (!resProcess.getPassed()) {
        ...
        Command cmd = buildCommand(sessionToken, basicFilenameNoPrefix, serverDirectory, localUsername);
    ...
    public Response processResponse(Command cmd, Response inResponse, String[] args) {
        ...
        if(ProtocolConstants.COMMAND_RETRIEVE_FILE.equals(cmd.getCommandName()) && inResponse.getPassed()){
     ...
     */
     public Command buildCommand(String sessionToken, String basicModuleName, 
         ...
         Command cmd = null;
         cmd = new Command(ProtocolConstants.COMMAND_RETRIEVE_FILE);
