 ...
 *
 */
public class MdfReferencesCheck extends ReferencesCheck {
	/**
	 ...
	 */
	public MdfReferencesCheck(EcoreUtil.CrossReferencer referencer) {
		super(referencer);
	}
	...
	protected boolean shouldBeIgnored(EReference reference) {
		//DS-3257 - look for order change of enumeration values
		if (reference == MdfPackage.eINSTANCE.getMdfEnumeration_Values()) {
			return false;
		}
		...
		for (final EObject added : new ArrayList<EObject>(addedReferences)) {
			for (final EObject deleted : deletedReferences) {
				...
				if (matched != null) {
					...
					final double uriSimilarity = MdfResourceSimilarity.computeURISimilarity(EcoreUtil
