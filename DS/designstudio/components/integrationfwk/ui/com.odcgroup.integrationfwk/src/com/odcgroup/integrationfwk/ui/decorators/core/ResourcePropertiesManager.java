 ...
 * 
 * 
 * Utility class to add, modify, get the persistent property of the resource.
 * 
 */
public class ResourcePropertiesManager {
	 ...
	 *            IResource object
	 * @param localName
	 *            qualifier name of the persistent property
	 * @param value
	 ...
	 *            IResource Object
	 * 
	 * @return the image decoration as List. For eg: If the file has a read only
	 *         property set, it returns a list with Lock as the only element
	 */
	...
	public static String getSuffix(IResource resource) {
		...
		QualifiedName suffixQualifiedName = PersistentPropertyTypes
	...
	private static boolean isBusy(IResource resource) {
		...
		QualifiedName busyQualifiedName = PersistentPropertyTypes.getInstance()
				.getBusyQualifiedName();
		try {
			// Get the Busy Value
	...
	/**
	 * Method which helps to remove the persistent property from the given
	 * resource.
	 * 
	 * @param resource
	 *            - instance of {@link IResource}
	 */
	public static void removePersistentProperty(IResource resource) {
		if (resource == null) {
			return;
		}
		// Get the Suffix Qualified Name
		...
		QualifiedName busyQualifiedName = PersistentPropertyTypes.getInstance()
				.getBusyQualifiedName();
		try {
			resource.setPersistentProperty(suffixQualifiedName, null);
			resource.setPersistentProperty(busyQualifiedName, null);
	 ...
	 * String returnText = ""; String prefixText = "";
	 * 
	 * try { prefixText = getPrefix(resource); if(prefixText != null &&
	 * prefixText.length() != 0) {
	 * 
	 * 
	 * 
	 * }
	 * 
	 * if(getPrefix(resource) ) { return getOwner(resource); } return null; }
