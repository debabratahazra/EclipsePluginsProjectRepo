...
/**
 * PropertyFilter implementation for MatrixAxis widget
 *
 * @author pkk
 *
 */
public class MatrixAxisPropertyFilter implements PropertyFilter {
	...
	public List<Property> filter(Widget widget) {
		...
		if (axis.isXAxis()) {
			allowedTypes.remove(mm.findPropertyType(library, "maxRows"));
		} else {
			allowedTypes.remove(mm.findPropertyType(library, "maxColumns"));
			allowedTypes.remove(mm.findPropertyType(library, "axis-halign"));			
		}
