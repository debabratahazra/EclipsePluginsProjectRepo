 ...
 *
 */
public class MatrixSimplifiedEnabledConditionTest extends AbstractPageDesignerUnitTest {
	 ...
	 */
	@Test
	public void testDS3804ValidationRuleSimplifiedEnablement() throws PartInitException {
		...
		if (!list.isEmpty()) {
			...
			WidgetEditPart compItem = (WidgetEditPart)exchildren.get(0);
			rep.getViewer().select(compItem);
		
			
			String expectedErrorMsg = "Attribute [instrument] must be dropped in " +
					"the cell or should be part of matrix extra columns";
