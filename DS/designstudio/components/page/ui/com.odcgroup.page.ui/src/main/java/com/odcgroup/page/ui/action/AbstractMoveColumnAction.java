...
/**
 * Base class action for moving column inside a table widget
 * 
 * @author Alexandre Jaquet
 *
 */
abstract public class AbstractMoveColumnAction extends AbstractGenericAction implements UpdateAction {
	 ...
	 */
	protected AbstractMoveColumnAction(ActionParameters parameters, WidgetEditPart oldEditPart) {
		super(parameters);
	...
	protected Widget getParentWidget() {
		...
		if (w.getTypeName().equals(WidgetTypeConstants.TABLE) || w.getTypeName().equals(WidgetTypeConstants.CDM_TABLE)) {
			return w;
		}
		return oldEditPart.getWidget().getParent().getParent();
	}	
}
