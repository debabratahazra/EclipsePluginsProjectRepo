package com.odcgroup.page.ui.command.handler;
...
/**
 * Handler of the command that does the page designer save and generate code
 * @author yan
 * @author amc
 */
public class SaveAndGenerateHandler extends AbstractHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		if(file != null) {
			if(isFragment(file)) {
				displayCannotGenerateFragmentDialog();
			}
			else {
	...
	private void displayCannotGenerateFragmentDialog() {
		Shell shell = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell();
		MessageDialog.openInformation(shell, 
				"Cannot generate fragment", 
				"Fragments can not be generated (Ctrl-G), only saved (Ctrl-S). Please re-generate the module(s) which include this fragment");
	}
	
	private boolean isFragment(IFile file) {
		return ModelUtils.isFragment(file);
	}
