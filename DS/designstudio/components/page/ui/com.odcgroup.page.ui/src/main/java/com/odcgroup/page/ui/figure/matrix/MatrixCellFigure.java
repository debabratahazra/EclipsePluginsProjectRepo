 ...
 *
 */
public class MatrixCellFigure extends BoxFigure {
	
	/** The border Color. */
	private Color borderColor;
	private static final String MATRIX_CELL_FIGURE__BORDER_COLOR = "MATRIX_CELL_FIGURE__BORDER_COLOR";

	/**
	 * @param widget
	 * @param figureContext
	 */
	public MatrixCellFigure(Widget widget, FigureContext figureContext) {
	 ...
	 * @return
	 */
	public IMatrixCell getMatrixCell() {
		return MatrixHelper.getMatrixCell(getWidget());
	}
	...
	protected void drawCaption(Graphics graphics) {	
		String caption = "";
		if (getMatrixCell().isLastCell()) {
			caption = "Last Cell";
		} else if (getMatrixCell().isLastColumn()) {
			caption = "Last Column";
		} else if (getMatrixCell().isLastRow()) {
			caption = "Last Row";
		}
		decorateColumnHeader(graphics, caption);			
	}	
	...
	public Color getBorderColor() {
		if (borderColor == null) {
			borderColor = PageUIPlugin.getColor(MATRIX_CELL_FIGURE__BORDER_COLOR);
		}
		return borderColor;
	}
	...
	private void initialize(){
	    PageUIPlugin.setColorInRegistry(MATRIX_CELL_FIGURE__BORDER_COLOR, new RGB(0, 128, 255));
	}
}
