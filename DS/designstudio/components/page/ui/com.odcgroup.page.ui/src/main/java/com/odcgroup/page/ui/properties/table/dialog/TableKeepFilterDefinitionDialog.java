...
import com.odcgroup.page.ui.command.table.InserTableKeepFilterCommand;
import com.odcgroup.page.ui.command.table.UpdateTableKeepFilterCommand;
 ...
 * 
 */
public class TableKeepFilterDefinitionDialog extends TableTransformDialog {
	 ...
	 * @param table
	 */
	public TableKeepFilterDefinitionDialog(Shell parentShell, CommandStack commandStack, ITable table) {
	 ...
	 * @param filterForEdit 
	 */
	public TableKeepFilterDefinitionDialog(Shell parentShell, CommandStack commandStack, ITable table, ITableKeepFilter filterForEdit) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		columnCombo = new StringValueCombo(body, TableDomainObjectUtil.getDomainAttributeNames(table),
				getFilteredColumns(table), isEditMode(), true);
	 ...
	 * @return List
	 */
	private List<String> getFilteredColumns(ITable table) {
		List<String> list = new ArrayList<String>();
		for (ITableKeepFilter filter : table.getKeepFilters()) {
	...
	protected BaseCommand getCommand() {
		...
		if (isEditMode()) {		
			...
			cmd = new UpdateTableKeepFilterCommand(table, filterForEdit, oldFilterForEdit);			
		} else {
			...
			cmd = new InserTableKeepFilterCommand(table, filter);
		}		
		return cmd;
	}
