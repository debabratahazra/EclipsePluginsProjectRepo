 ...
 */
package com.odcgroup.pageflow.editor.diagram.parsers;
...
public abstract class AbstractParser implements IParser {
	...
	protected Object[] getValues(EObject element) {
		...
		values[0] = getValue(element, features[i]);
	...
	protected ICommand getParseCommand(IAdaptable adapter, Object[] values, int flags) {
		...
		command.compose(getModificationCommand(element, features[i], values[0]));
	...
	protected IParserEditStatus validateNewValues(Object[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (value instanceof InvalidValue) {
				return new ParserEditStatus(PageflowDiagramEditorPlugin.ID, IParserEditStatus.UNEDITABLE, value
						.toString());
			}
		}
		...
		if (type instanceof EDataType) {
			...
			if (Boolean.TYPE.equals(iClass)) {
				if (value instanceof Boolean) {
					...
					value = new InvalidValue(NLS.bind(Messages.AbstractParser_UnexpectedValueTypeMessage, iClass
							.getName()));
				}
				...
				if (value instanceof String) {
					...
					value = new InvalidValue(NLS.bind(Messages.AbstractParser_UnexpectedValueTypeMessage, String.class
							.getName()));
				}
			}
		}
