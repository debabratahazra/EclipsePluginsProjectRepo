 ...
 * @generated
 */
public class PageflowValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof PageflowDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && PageflowEditPart.MODEL_ID.equals(PageflowVisualIDRegistry.getModelID(view));
	}
	...
	private static void refreshDecorators(String viewId, Diagram diagram) {
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					TransactionalEditingDomain domain = TransactionUtil.getEditingDomain(fdiagram);
					if (domain != null) {
					...
					PageflowDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				PageflowDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			...
			if (view == null || view.eResource() == null || view.getElement() == null) {
				return;
			}
			...
			URI elementUri = EcoreUtil.getURI(view.getElement());
			if(view.eResource().getResourceSet().getURIConverter() instanceof ModelURIConverter) {
				elementUri = ModelURIConverter.toResourceURI(elementUri);
			}
			...
			try {
				...
				PageflowDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				PageflowDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
