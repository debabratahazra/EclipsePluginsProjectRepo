...
public class ToggleIDNameDialog extends AbstractTitleAreaDialog {
	 ...
	 * @param parentShell
	 */
	public ToggleIDNameDialog(Shell parentShell) {
		super(parentShell);
	}
	...
	public void create() {
		super.create();
		this.setTitle(Messages.ToggleIDNameDialogTitle);
		this.setMessage(Messages.ToggleIDNameDialogMessage);
	}
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText(Messages.ToggleIDNameDialogShellTxt);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		fullClassBtn.setText(Messages.ShowIDBtnLabel);
		if (currVal) {
			fullClassBtn.setSelection(true);
		}

		final Button actionBtn = new Button(radio, SWT.RADIO);
		actionBtn.setText(Messages.ShowNameBtnLabel);
		if (!currVal) {
	...
	protected void okPressed() {
		...
		for (int i = 0; i < child.length; i++) {
			...
			if (control instanceof Button) {
				...
				boolean val = false;
				if(button.getText().equals(Messages.ShowIDBtnLabel)){
					val = true;
				} else {
					val = false;
				}
				if ((button.getStyle() & SWT.RADIO) != 0
						&& button.getSelection()) {
					getPreferenceStore().setValue(PageflowPreferenceConstants.PREF_SHOWID_LABEL,  val);
				}
			}
		}
		super.okPressed();
