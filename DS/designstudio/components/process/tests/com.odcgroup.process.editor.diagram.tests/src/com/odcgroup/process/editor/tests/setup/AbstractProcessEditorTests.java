 ...
 *
 */
public abstract class AbstractProcessEditorTests  {
    ...
    protected void openDiagram() throws Exception {
        ...
        setDiagramWorkbenchPart((IDiagramWorkbenchPart) getWorkbenchPage().openEditor(
                new FileEditorInput(getDiagramFile()), ProcessDiagramEditor.ID));
    ...
    protected void saveDiagram() {
        if (getDiagramWorkbenchPart() instanceof IEditorPart) {
            getWorkbenchPage().saveEditor((IEditorPart) getDiagramWorkbenchPart(), false);
            flushEventQueue();
        }
    }
    ...
    public boolean closeDiagram() {
		...
		if (diagramWorkbenchPart != null // only close if it was opened
			...
			closed = getWorkbenchPage().closeEditor((IEditorPart)getDiagramWorkbenchPart(), false);
