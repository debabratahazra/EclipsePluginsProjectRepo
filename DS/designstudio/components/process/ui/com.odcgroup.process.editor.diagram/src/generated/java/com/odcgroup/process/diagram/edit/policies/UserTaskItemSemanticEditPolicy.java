 ...
 * @generated
 */
public class UserTaskItemSemanticEditPolicy extends ProcessBaseItemSemanticEditPolicy {
	...
	protected boolean allowOnlyOneOutgoingTransition(EObject source) {
		...
		if (containsUserTask(transitions, source, true)) {
			return false;
		}
		return true;
	}
	...
	protected boolean allowOnlyOneInComingTransition(EObject target) {
		...
		List transitions = process.getTransitions();
		if (containsUserTask(transitions, target, false)) {
			return false;
		}
		return true;
	 ...
	 * @return
	 */
	protected boolean containsUserTask(List<?> transitions, EObject eObject, boolean source) {
