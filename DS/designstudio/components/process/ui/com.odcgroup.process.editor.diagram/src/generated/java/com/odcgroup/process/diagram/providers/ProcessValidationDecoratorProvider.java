 ...
 * @generated
 */
public class ProcessValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && ProcessEditPart.MODEL_ID.equals(ProcessVisualIDRegistry.getModelID(view));
	}
	...
	/**
	 * put a nullpointer check for txDomain to avoid "Decorator refresh failure" exception
	 * @generated NOT
	 */
	private static void refreshDecorators(String viewId, Diagram diagram) {
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					TransactionalEditingDomain txDomain = TransactionUtil.getEditingDomain(fdiagram);
					if (txDomain != null) {
					...
					ProcessDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public void refresh() {
			...
			try {
				...
				ProcessDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
