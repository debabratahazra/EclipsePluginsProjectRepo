package com.odcgroup.service.gen.t24.internal.generator;

import java.io.Writer;

import org.apache.velocity.Template;
import org.apache.velocity.VelocityContext;

import com.odcgroup.service.gen.t24.internal.data.OperationDescriptor;
import com.odcgroup.service.gen.t24.internal.data.ServiceDescriptor;
import com.odcgroup.service.gen.t24.internal.exceptions.LoadTemplateException;

/**
 * Generates BASIC API subroutines from a {@link ServiceDescriptor}. 
 * @author jannadani
 *
 */
public class BasicApiGenerator extends ServiceGenerator {

	public static final String TEMPLATE = "BasicApi.vm";
	private Template template;

	public BasicApiGenerator(VelocityTemplateLoader loader) throws LoadTemplateException {
		template = loader.loadTemplate(TEMPLATE);
	}
	
	/* (non-Javadoc)
	 * @see com.temenos.t24.modelgen.ServiceGenerator#generate(com.temenos.t24.modelgen.ServiceDescriptor, java.io.Writer)
	 */
	public void generate(ServiceDescriptor serviceDescriptor, Writer writer, String path) {
		for (OperationDescriptor operationDescriptor : serviceDescriptor.getGenOperations()) {
			generateOperation(serviceDescriptor, operationDescriptor, writer, path);
		}
	}
	
	private void generateOperation(ServiceDescriptor service, OperationDescriptor operation, Writer writer, String path) {
		VelocityContext ctx = new VelocityContext();
		String serviceName = service.getName();
		try {			
			ctx.put("service", serviceName);
			ctx.put("classes", service.getClassDefDescriptors());
			ctx.put("operation", operation);
			if (operation.stereotypeIsApplied("integrationFlowSupportable")) {
				ctx.put("argsName", operation.getParamNamesAsStringForJBC("@AM"));
				ctx.put("argsValue", operation.getParamValuesAsStringForJBC());
			}
			if (writer == null) {
				String dirName = path;
				String apiFileName = serviceName + "." + operation.getLowerName() + ".b";

				writer = createWriter(dirName, apiFileName);
			}
			System.out.println("Merging Template - OperationApiGenerator: " + TEMPLATE);
			template.merge(ctx, writer);
			System.out.println("Done");
			writer.close();
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		}
	}
}
