 ...
 *
 */
public class JavaProxyServiceAPIGenerator extends ServiceGenerator {

	private static final String TEMPLATE = "Java_ProxyServiceAPI.vm";
	
	private Template template;
	
	public JavaProxyServiceAPIGenerator(VelocityTemplateLoader loader) throws LoadTemplateException {
	...
	public void generate(ServiceDescriptor service, Writer writer, String path) {
		...
		try {
			...
			if (writer == null) {
				...
				String apiFileName = service.getName() + "ProxyAPI.java";
		
				writer = createWriter(dirName, apiFileName);
			}
			System.out.println("Merging Template - Java_ProxyServiceAPIGenerator: " + TEMPLATE);
