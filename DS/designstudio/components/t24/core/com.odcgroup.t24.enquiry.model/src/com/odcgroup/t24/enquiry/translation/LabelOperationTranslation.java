 ...
 *
 */
public class LabelOperationTranslation extends BaseVersionTranslation {

	private LabelOperation labelOperation;
	/**
	 * @param provider
	 * @param project
	 */
	protected LabelOperationTranslation(ITranslationProvider provider, IProject project,LabelOperation tool) {
		super(provider, project);
		this.labelOperation = tool;
	}
	...
	public String getText(ITranslationKind kind, Locale locale) throws TranslationException {
		...
		case NAME: {
			LocalTranslation localTranslation = findTranslation(locale, (LocalTranslations) labelOperation.getLabel());
	...
	public String setText(ITranslationKind kind, Locale locale, String newText) throws TranslationException {
		...
		switch (kind) {
		case NAME: {
			if (labelOperation.getLabel() == null) {
				labelOperation.setLabel(TranslationDslPackage.eINSTANCE.getTranslationDslFactory().createLocalTranslations());
			}
			oldText = setTranslation((LocalTranslations) labelOperation.getLabel(), locale, newText);
	...
	public String delete(ITranslationKind kind, Locale locale) throws TranslationException {
		...
		case NAME: {
			localTranlations = (LocalTranslations) labelOperation.getLabel();
