 ...
 *
 */
public class SelectionTranslation extends BaseVersionTranslation {
	 ...
	 * @param element 
	 */
	protected SelectionTranslation(ITranslationProvider provider, IProject project, Selection element) {
	...
	public String getText(ITranslationKind kind, Locale locale) throws TranslationException {
		...
		switch (kind) {
			...
			case NAME: {
				LocalTranslation localTranslation = findTranslation(locale, (LocalTranslations)selection.getLabel());
		...
		switch (kind) {
			...
			case NAME: {
				if (selection.getLabel() == null) {
					selection.setLabel(TranslationDslPackage.eINSTANCE.getTranslationDslFactory().createLocalTranslations());
				}
				oldText = setTranslation((LocalTranslations)selection.getLabel(), locale, newText);
	...
	public String delete(ITranslationKind kind, Locale locale) throws TranslationException {
		...
		switch (kind) {
			...
			case NAME: {
				localTranlations=(LocalTranslations)selection.getLabel();
