 ...
 *
 */
public class FieldMoveDownAction extends AbstractMoveAction {

	public static final String ACTION_ID = "FIELD_MOVE_DOWN";
	private Request request;

	/**
	 * @param part
	 */
	public FieldMoveDownAction(IWorkbenchPart part) {
		super(part);
		setId(ACTION_ID);
		setText("Move Down");
	...
	protected boolean calculateEnabled() {
		...
		if (selection != null && selection.size() == 1) {
			...
			if (obj instanceof OutputFieldEditPart) {
				...
				return (enquiry.getFields().size() != index+1);
			}
		}
		return false;
	}
