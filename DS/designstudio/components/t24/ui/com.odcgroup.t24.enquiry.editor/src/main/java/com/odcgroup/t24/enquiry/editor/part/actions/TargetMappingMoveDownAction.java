 ...
 * 
 */
public class TargetMappingMoveDownAction extends AbstractMoveAction {

	public static final String ACTION_ID = "TARGET_MAPPING_MOVE_DOWN";
	private Request request;

	/**
	 * @param part
	 */
	public TargetMappingMoveDownAction(IWorkbenchPart part) {
		super(part);
		setId(ACTION_ID);
		setText("Move Down");
	...
	protected boolean calculateEnabled() {
		...
		if (selection != null && selection.size() == 1) {
			...
			if (obj instanceof TargetMappingEditPart) {
				...
				return ((target.getMappings().size() - 1) > index);
			}
		}
		return false;
	}
