 ...
 * 
 */
public class ToolMoveDownAction extends AbstractMoveAction {

	public static final String ACTION_ID = "TOOL_MOVE_DOWN";
	private Request request;

	/**
	 * @param part
	 */
	public ToolMoveDownAction(IWorkbenchPart part) {
		super(part);
		setId(ACTION_ID);
		setText("Move Down");
	...
	protected boolean calculateEnabled() {
		...
		if (selection != null && selection.size() == 1) {
			...
			if (obj instanceof ToolEditPart) {
				...
				return ((enquiry.getTools().size() - 1) > index);
			}
		}
		return false;
	}
