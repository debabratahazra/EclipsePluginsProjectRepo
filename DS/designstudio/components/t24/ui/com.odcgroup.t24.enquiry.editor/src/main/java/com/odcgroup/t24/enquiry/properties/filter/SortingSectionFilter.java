package com.odcgroup.t24.enquiry.properties.filter;

import java.util.ArrayList;
import java.util.List;

import org.eclipse.emf.ecore.EObject;
import org.eclipse.emf.ecore.EStructuralFeature;
import org.eclipse.ui.views.properties.IPropertyDescriptor;

import com.odcgroup.t24.enquiry.enquiry.Enquiry;
import com.odcgroup.t24.enquiry.enquiry.FixedSort;
import com.odcgroup.t24.enquiry.properties.descriptors.FixedSortPropertyDescriptor;

/**
 * TODO: Document me!
 *
 * @author satishnangi
 *
 */
public class SortingSectionFilter extends AbstractPropertyFilter {

	@Override
	public List<EStructuralFeature> getFilterProperties(EObject eObject) {
		List<EStructuralFeature> list = new ArrayList<EStructuralFeature>();
		return list;
	}
	
	@Override
	protected IPropertyDescriptor[] customPropertyDecriptors(IPropertyDescriptor[] propertyDescriptors, EObject model) {
		List<IPropertyDescriptor> custompropertyList = new ArrayList<IPropertyDescriptor>();
		if (model instanceof Enquiry) {
			Enquiry enquiry = (Enquiry) model;
			List<FixedSort> fixedSortList = enquiry.getFixedSorts();
			for (FixedSort fixedSort : fixedSortList) {
				custompropertyList.add(new FixedSortPropertyDescriptor(fixedSort));
			}
		}
		return custompropertyList.toArray(new IPropertyDescriptor[0]);
	}
}
