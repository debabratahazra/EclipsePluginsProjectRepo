...
import com.odcgroup.t24.packager.launcher.T24TafcPackagerLauncherHelper;
 ...
 *
 */
public class CreateTafcPackageAction extends Action implements IObjectActionDelegate {

	private static final int EXPECTED_NB_LINES_TO_SUCCESS = 12300;

	private static Logger logger = LoggerFactory.getLogger(CreateTafcPackageAction.class);

	private IProject packagerProject;
	
	public CreateTafcPackageAction() {
		setText("Create TAFC Package");
	 ...
	 * 
	 */
	private void createTAFCPackage() {
		if (!MessageDialog.openConfirm(Display.getCurrent().getActiveShell(),  
				"TAFC Packager Launch confirmation", 
				"You are about to build a TAFC Package. This process may take several minutes. Click OK to proceed, or Cancel to abort.")) {
			return;
		}
		
		Job job = new WorkspaceJob("Generating TAFC Package") {
			public IStatus runInWorkspace(final IProgressMonitor monitor) {
				monitor.beginTask("Running TAFC Package generation (this may take a few minutes)", 100);
				
				final ILaunch launch;
				try {
					launch = T24TafcPackagerLauncherHelper.getHelper().launchPackager(packagerProject);
					...
					if (!launch.isTerminated()) {
						try {
							...
							logger.error("Unable to terminate the TAFC Package Launch", e);
						}
					} else {
						...
						if (listener.isBuildSuccessful()) {
							if (listener.getZipPackageGenerated() != null) {
								try {
									T24TafcPackagerLauncherHelper.getHelper().openGeneratedTAFCPackage(listener.getZipPackageGenerated());
								} catch (final UnableToOpenExplorerException e) {
									openError("TAFC Packager Generation Problem", 
