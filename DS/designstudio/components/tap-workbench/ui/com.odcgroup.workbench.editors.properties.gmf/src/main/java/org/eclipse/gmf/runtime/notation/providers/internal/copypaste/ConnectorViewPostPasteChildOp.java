 ...
 * @author Yasser Lulu
 */
class ConnectorViewPostPasteChildOp
	...
	ConnectorViewPostPasteChildOp(
			ConnectorViewPasteOp connectorViewPasteOperation,
			boolean pasteSemanticElement) {
		super(connectorViewPasteOperation, EMPTY_ARRAY);
		this.pasteSemanticElement = pasteSemanticElement;
	}

	private ConnectorViewPasteOp getConnectorViewPasteOp() {
		return (ConnectorViewPasteOp) getPasteChildOperation();
	}
	...
	private EObject doPaste()
		...
		View sourceView = getConnectorViewPasteOp().getSourceView();
		View targetView = getConnectorViewPasteOp().getTargetView();
		...
		if (sourceViewContainer.equals(targetViewContainer) == false) {
			...
			Diagram sourceViewDiagram = NotationClipboardOpHelper
				.getContainingDiagram((View) sourceViewContainer);
			Diagram targetViewDiagram = NotationClipboardOpHelper
		...
		Edge connectorView = getConnectorViewPasteOp()
			.getConnectorView();
		...
		Diagram pasteTargetDiagram = NotationClipboardOpHelper
			.getContainingDiagram((View) sourceViewContainer);
	...
	private List getSemanticElementPasteOperations() {
		if (semanticElementPasteOperations == null) {
			...
			// been pasted already
			EObject semanticPasteTarget = NotationClipboardOpHelper
				.getSemanticPasteTarget(getConnectorViewPasteOp()
					.getSourceView());
