...
public class MenuGeneratorComponent extends AbstractWorkflowComponent {
	...
	protected void invokeInternal(WorkflowContext ctx, ProgressMonitor monitor, Issues issues) {	
		IMenuGenerator generator = getCompiler();
		if (generator == null) {
			return;
		}
		...
		for (String slot : slots) {
			...
			if (object instanceof MenuRoot) {
				generator.doGenerate((MenuRoot)object, fsa, getGlobalVars(ctx));
			}
		}
	}
	 ...
	 * @return
	 */
	protected IMenuGenerator getCompiler() {
		return injector.getInstance(IMenuGenerator.class);
	}
