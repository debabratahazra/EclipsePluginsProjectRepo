 ...
 * </p>
 */
public abstract class LocaleListEditor extends FieldEditor {
     ...
     */
    protected LocaleListEditor(String name, String labelText, Composite parent) {
        init(name, labelText);
...
//    }
    ...
    public List getListControl(Composite parent) {
        if (list == null) {
            list = new List(parent, SWT.BORDER | /*SWT.SINGLE*/ SWT.MULTI | SWT.V_SCROLL
                    | SWT.H_SCROLL);
    ...
    protected abstract void itemsRemoved(String[] items);
    ...
    private void removePressed() {
        ...
        if (indices.length > 0) {
	        ...
	        for (int kx = indices.length-1; kx >= 0; --kx) {
	        	items[kx] = list.getItem(indices[kx]);
	        	list.remove(indices[kx]);
	        }
	        itemsRemoved(items);
	        itemsChanged();
        }
...
//        if (index >= 0) {
//        	String[] items = {list.getItem(index)};
//            list.remove(index);
//            itemsRemoved(items);
//            selectionChanged();
//        }
