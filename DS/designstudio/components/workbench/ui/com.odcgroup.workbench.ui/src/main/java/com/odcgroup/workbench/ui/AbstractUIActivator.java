...
public abstract class AbstractUIActivator extends AbstractUIPlugin {
	...
	protected static Map<Class,AbstractUIActivator> plugins = new HashMap<Class,AbstractUIActivator>();

    protected static Map<Class,ResourceBundle> resourceBundles = new HashMap<Class,ResourceBundle>();
	 ...
	 */
	public static AbstractUIActivator getDefault(Class clazz) {
		AbstractUIActivator plugin = plugins.get(clazz);
		if(plugin==null) {
			try {
				plugin = (AbstractUIActivator) clazz.newInstance();
			} catch (Exception e) {
				e.printStackTrace();
			}
	 ...
	 * This method has to be implemented by all subclasses of this abstract activator
	 * that provide text resources.
     ...
     *
     * @param key the key of the message to be returned
     * @param params string arguments to be inserted into the message
     */
