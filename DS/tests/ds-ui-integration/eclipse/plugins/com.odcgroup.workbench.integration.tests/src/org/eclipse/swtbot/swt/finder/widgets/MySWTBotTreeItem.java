 ...
 * @version $Id$
 */
public class MySWTBotTreeItem extends MyAbstractSWTBot<TreeItem> {
...
//	private static final int	collapseKey	= SWT.getPlatform().equals("gtk") ? SWT.KEYPAD_SUBTRACT : SWT.ARROW_LEFT;
	 ...
	 */
	public MySWTBotTreeItem(final TreeItem treeItem) throws WidgetNotFoundException {
		this(treeItem, null);
	}
	 ...
	 * @throws WidgetNotFoundException if the widget is <code>null</code> or widget has been disposed.
	 */
	public MySWTBotTreeItem(final TreeItem treeItem, SelfDescribing description) throws WidgetNotFoundException {
	...
	public String cell(final int column) {
		...
		int columnCount = new MySWTBotTree(tree).columnCount();
	 ...
	 */
	public MySWTBotTreeItem expand() {
		assertEnabled();
	 ...
	 */
	public MySWTBotTreeItem collapse() {
		assertEnabled();
	 ...
	 */
	public MySWTBotTreeItem expandNode(final String... nodes) {
		Assert.isNotEmpty((Object[]) nodes);
		assertEnabled();

		MySWTBotTreeItem item = this;
	 ...
	 */
	public MySWTBotTreeItem collapseNode(final String nodeText) {
		assertEnabled();
	 ...
	 * @since 2.0
	 */
	public MySWTBotTreeItem getNode(final String nodeText, final int index) {
		List<MySWTBotTreeItem> nodes = getNodes(nodeText);
	 ...
	 * @since 2.0
	 */
	public List<MySWTBotTreeItem> getNodes(final String nodeText) {
		List<MySWTBotTreeItem> foundItems = syncExec(new ListResult<MySWTBotTreeItem>() {
			public List<MySWTBotTreeItem> run() {
				TreeItem[] items = widget.getItems();
				List<MySWTBotTreeItem> results = new ArrayList<MySWTBotTreeItem>();
				for (TreeItem treeItem : items) {
					if (treeItem.getText().equals(nodeText))
						results.add(new MySWTBotTreeItem(treeItem, new TextDescription("Tree node with text: " + nodeText)));
				}
				return results;
			}
		});
	 ...
	 * @since 1.2
	 */
	public MySWTBotTreeItem getNode(final String nodeText) {
	 ...
	 * @since 1.0
	 */
	public MySWTBotTreeItem select() {
		assertEnabled();
	 ...
	 * @since 1.2
	 */
	public MySWTBotTreeItem doubleClick() {
		assertEnabled();
		asyncExec(new VoidResult() {
			public void run() {
				tree.setSelection(widget);
	 ...
	 * @since 1.0
	 */
	public MySWTBotTreeItem select(final String... items) {
		assertEnabled();
	 ...
	 * @since 1.0
	 */
	public MySWTBotTreeItem select(final String item) {
	...
	public SWTBotMenu contextMenu(String text) {
		new MySWTBotTree(tree).waitForEnabled();
		select();
	...
	@Override
	public SWTBotMenu contextMenu(String text, int index) {
		new MySWTBotTree(tree).waitForEnabled();
		select();
		notifyTree(SWT.MenuDetect);
		return super.contextMenu(tree, text, index);
	}
	...
	protected void assertEnabled() {
		new MySWTBotTree(tree).assertEnabled();
	}
