...
public class HighlightMixedMode {
	...
	public void highlightLine(String address) {
		/**
		 * @param Hex address for highlighting the text from .MXM file.
		 */
		...
		if(address.equalsIgnoreCase("0000")){
			...
			//Convert from .DSM to .MXM extension except LIBRARY.DSM.
			if(filePath.toUpperCase().contains("LIBRARY.DSM"));
			else{
				int lIndex = filePath.lastIndexOf(".");
				filePath = filePath.substring(0, lIndex) + ".MXM";			
			}						
