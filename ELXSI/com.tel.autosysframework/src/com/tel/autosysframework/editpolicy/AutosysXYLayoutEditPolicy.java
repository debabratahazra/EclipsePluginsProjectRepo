...
import com.tel.autosysframework.commands.AddCommand;
import com.tel.autosysframework.commands.ChangeGuideCommand;
import com.tel.autosysframework.commands.CloneCommand;
import com.tel.autosysframework.commands.CreateCommand;
import com.tel.autosysframework.commands.SetConstraintCommand;
...
public class AutosysXYLayoutEditPolicy
	extends org.eclipse.gef.editpolicies.XYLayoutEditPolicy
{

public AutosysXYLayoutEditPolicy(XYLayout layout) {
	super();
	setXyLayout(layout);
}

protected Command chainGuideAttachmentCommand(
		Request request, AutosysSubpart part, Command cmd, boolean horizontal) {
	Command result = cmd;
...
protected Command chainGuideDetachmentCommand(Request request, AutosysSubpart part,
...
protected Command createAddCommand(Request request, EditPart childEditPart, 
		Object constraint) {
	AutosysSubpart part = (AutosysSubpart)childEditPart.getModel();
	Rectangle rect = (Rectangle)constraint;

	AddCommand add = new AddCommand();
	add.setParent((AutosysDiagram)getHost().getModel());
	add.setChild(part);
	add.setLabel("Reaprenting AutosysSubpart");
	add.setDebugLabel("AutosysXYEP add subpart");//$NON-NLS-1$

	SetConstraintCommand setConstraint = new SetConstraintCommand();
	setConstraint.setLocation(rect);
	setConstraint.setPart(part);
	setConstraint.setLabel("Reaprenting AutosysSubpart");
	setConstraint.setDebugLabel("AutosysXYEP setConstraint");//$NON-NLS-1$
...
protected Command createChangeConstraintCommand(EditPart child, Object constraint) {
    ...
    locationCommand.setPart((AutosysSubpart)child.getModel());
...
protected Command createChangeConstraintCommand(ChangeBoundsRequest request, 
	...
	AutosysSubpart part = (AutosysSubpart)child.getModel();
	cmd.setPart(part);
...
protected EditPolicy createChildEditPolicy(EditPart child) {
	/*if (child instanceof AutosysDiagramEditPart 
			|| child instanceof OutputEditPart) {*/
		ResizableEditPolicy policy = new /*Autosys*/ResizableEditPolicy();
		...
		return policy;
	//}
/*else if (child instanceof AutosysLabelEditPart) {
		ResizableEditPolicy policy = new AutosysResizableEditPolicy();
		policy.setResizeDirections(PositionConstants.EAST | PositionConstants.WEST);
		return policy;
	}*/
	
	//return new AutosysResizableEditPolicy();
}
...
/*protected IFigure createSizeOnDropFeedback(CreateRequest createRequest) {
	...
	else if (createRequest.getNewObject() instanceof AutosysFlowContainer)
		figure = new AutosysFlowFeedbackFigure();		
	else if (createRequest.getNewObject() instanceof AutosysLabel)
...
protected AutosysGuide findGuideAt(int pos, boolean horizontal) {
	...
	return (AutosysGuide)provider.getGuideAt(pos);
}
...
protected Command getCloneCommand(ChangeBoundsRequest request) {
	...
	clone.setParent((AutosysDiagram)getHost().getModel());
	...
	while (i.hasNext()) {
		...
		clone.addPart((AutosysSubpart)currPart.getModel(), 
...
protected Command getCreateCommand(CreateRequest request) {
	...
	create.setParent((AutosysDiagram)getHost().getModel());
	AutosysSubpart newPart = (AutosysSubpart)request.getNewObject();
	create.setChild(newPart);
	Rectangle constraint = (Rectangle)getConstraintFor(request);
	create.setLocation(constraint);
	create.setLabel("Creating AutosysSubpart");
