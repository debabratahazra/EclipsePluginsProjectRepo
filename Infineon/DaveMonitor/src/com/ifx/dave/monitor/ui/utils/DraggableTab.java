...
/**
 * Provide the functionality of Drag and Drop the Tab
 * of Oscilloscope in a new Window or Existing Window
 * @author Hazra
 */
public class DraggableTab extends Tab {
                ...
                if (!t.isStillSincePress()) {
                    ...
                    if (insertData != null) {
                        ...
                        insertData.getInsertPane().selectionModelProperty()
                                .get().select(addIndex);
                        // Add Open Tab information for adding Tab in existing Window
                        addDetachableTabInformation(insertData.getInsertPane());
                        return;
                    }
                    if (!detachable) {
                        return;
                    ...
                    newStage.setOnCloseRequest(new EventHandler<WindowEvent>() {
                        ...
                        public void handle(WindowEvent event) {
                            ...
                            for (Iterator<Tab> iterator = tabs.iterator(); iterator
                                    .hasNext();) {
                                Tab tab = (Tab) iterator.next();
                                DragableTabController.addClosedTabId(Integer.parseInt(tab.getId()));
                                DragableTabController.removeOpenTabId(Integer.parseInt(tab.getId()));
                            }
                        }
                    });
                    ...
                    Scene childScene = styledScene(pane);
                    newStage.setScene(childScene);
                    newStage.initStyle(StageStyle.DECORATED);
                    newStage.setWidth(MainWindow.DRAG_WINDOW_WIDTH);
                    newStage.setHeight(MainWindow.DRAG_WINDOW_HEIGHT);
                    newStage.setX(t.getScreenX());
                    newStage.setY(t.getScreenY());

                    newStage.show();

                    // Add Open Tab Information while create new Window
                    addDetachableTabInformation(pane);
    ...
    protected void addDetachableTabInformation(TabPane pane) {
        ObservableList<Tab> tabs = pane.getTabs();
        for (Iterator<Tab> iterator = tabs.iterator(); iterator
                .hasNext();) {
            Tab tab = (Tab) iterator.next();
            DragableTabController.addOpenTabIds(Integer.parseInt(tab.getId()));
        }
    }

    protected Scene styledScene(TabPane pane) {
