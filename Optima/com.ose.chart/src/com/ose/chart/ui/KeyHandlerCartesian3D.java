...
class KeyHandlerCartesian3D extends KeyHandler
{
   static final float ZOOM_STEP = 10.0f;

   public KeyHandlerCartesian3D(ChartCanvas canvas, CartesianChart3D chart)
   {
      super(canvas, chart);
   }
   ...
   {
      CartesianChart3D chart = (CartesianChart3D) getChart();

      if (e.stateMask == 0)
      {
         if (e.character == 'a' || e.character == 'A')
         {
            chart.moveBackdrop(-1);
         }
         else if (e.character == 'z' || e.character == 'Z')
         {
            chart.moveBackdrop(1);
         }
         else if (e.keyCode == SWT.ARROW_UP)
         {
            chart.scrollSeries(1);
         }
