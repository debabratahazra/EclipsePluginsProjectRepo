...
public class DBGGetCPULoadReportsReply extends Message implements dbgserverinterfaceConstants{
   ...
   public DBGGetCPULoadReportsReply(int _measurementStatus, int _currentTimeTick, int _currentTimeUTick, int _reports[]) {
   ...
   public DBGGetCPULoadReportsReply(DataInputStream _s) throws IOException { signalNo = 33007; read(_s);}
   ...
   public final static void write(DataOutputStream _s, int _measurementStatus, int _currentTimeTick, int _currentTimeUTick, int _reports[]   ) throws IOException {
         int _i;
         _s.writeInt(DBGGETCPULOADREPORTSREPLY);
         int _size=20
            +4+(_reports.length*4)
            ;
         _s.writeInt(_size);
