...
public class DBGMonitorSendReceiveNotify extends Message implements dbgserverinterfaceConstants{
   public short state;
   ...
   public DBGMonitorSendReceiveNotify(short _state, byte _eventType, byte _notUsed, int _tick, int _utick, int _abstime, int _abstick, int _fromPid, int _toPid, int _signalNo, int _signalId, String _fromName, String _toName, byte _data[]) {
      state = _state;
   ...
   public DBGMonitorSendReceiveNotify(DataInputStream _s) throws IOException { signalNo = 32929; read(_s);}
   public final void sendMessage(DataOutputStream _s) throws IOException { write(_s, this.state, this.eventType, this.notUsed, this.tick, this.utick, this.abstime, this.abstick, this.fromPid, this.toPid, this.signalNo, this.signalId, this.fromName, this.toName, this.data);}
   public final static void write(DataOutputStream _s, short _state, byte _eventType, byte _notUsed, int _tick, int _utick, int _abstime, int _abstick, int _fromPid, int _toPid, int _signalNo, int _signalId, String _fromName, String _toName, byte _data[]   ) throws IOException {
         int _i;
         int _fromNameSize=_fromName.length()+1;
         int _toNameSize=_toName.length()+1;
         _s.writeInt(DBGMONITORSENDRECEIVENOTIFY);
         int _size=44
