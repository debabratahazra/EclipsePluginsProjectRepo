...
public class DBGWriteRegister extends Message implements dbgserverinterfaceConstants{
   public int pid;
   public int regVal[];


   public DBGWriteRegister(int _pid, int _regVal[]) {
      pid = _pid;
      regVal = _regVal;
   }

   public DBGWriteRegister(DataInputStream _s) throws IOException { signalNo = 33026; read(_s);}
   public final void sendMessage(DataOutputStream _s) throws IOException { write(_s, this.pid, this.regVal);}
   public final static void write(DataOutputStream _s, int _pid, int _regVal[]   ) throws IOException {
         int _i;
         _s.writeInt(DBGWRITEREGISTER);
         int _size=12
            +4+(_regVal.length*4)
            ;
         _s.writeInt(_size);
