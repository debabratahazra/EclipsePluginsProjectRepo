...
import org.eclipse.cdt.debug.core.cdi.model.ICDIVariable2;
...
{
   public static final String PLUGIN_ID = "com.ose.fmd.freescale.sc";
   ...
   private static class TargetProxy implements ITargetProxy
   {
      ...
      public long getAddress(String symbol) throws TargetProxyException
      {
         ...
         if (cdiTarget != null)
         {
            try
            {
               ICDIGlobalVariableDescriptor variableDescriptor =
                  cdiTarget.getGlobalVariableDescriptors(null, null, symbol);
               ICDIGlobalVariable variable =
                  cdiTarget.createGlobalVariable(variableDescriptor);
               // XXX: ICDIVariable2 is Freescale-specific API!
               if (variable instanceof ICDIVariable2)
               {
                  ICDIVariable2 var = (ICDIVariable2) variable;
                  String location = var.getLocation().getLocationString();
                  var.dispose();
                  // The location string is in the following format:
                  // <address>`<memory-space-id>, e.g. 0x12345678`Data.
                  int i = location.indexOf('`');
                  if (i > 0)
                  {
                     location = location.substring(0, i);
                  }
                  IAddress addr = new Addr64(location);
                  return addr.getValue().longValue();
               }
            }
            catch (CDIException e)
            {
               throw new TargetProxyException("Error looking up symbol "
                     + symbol, e);
            }
         }
