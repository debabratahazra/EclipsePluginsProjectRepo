...
/*
 * The gateway protocol detach command.
 */
class CommandDetach extends Command
{
   /* Request parameters */
   private int attref;
   ...
   /*
    * Create a new detach command object.
    *
    * @param attref  the attach reference.
    */
   CommandDetach(int attref)
   {
      this.attref = attref;
   }
   ...
   {
      out.writeInt(Command.DETACH_REQUEST);
      out.writeInt(4);
      out.writeInt(attref);
      out.flush();
   }
   ...
   {
      if (readHeader(in) == Command.DETACH_REPLY)
      {
         status = in.readInt();
      }
      else
