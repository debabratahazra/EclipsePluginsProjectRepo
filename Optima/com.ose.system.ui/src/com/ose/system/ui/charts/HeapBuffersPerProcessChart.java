...
public class HeapBuffersPerProcessChart extends AbstractBarChart
{
   private BarSeries usedSeries;

   public HeapBuffersPerProcessChart()
   {
      super("Buffers / Process",
            "The number of allocated heap buffers per process.",
   ...
   public void update(Object input)
   {
      ...
      for (int i = 0; i < buffers.length; i++)
      {
         ...
         key = new Integer(ownerId);

         cd = (ChartData) map.get(key);
         if (cd == null)
         {
            cd = new ChartData(ownerId, buffers[i].getOwnerName());
            cd.setNumBuffers(1);
            map.put(key, cd);
         }
         else
