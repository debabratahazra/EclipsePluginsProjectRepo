...
public class SignalsPerSignalSizeChart extends AbstractBarChart
{
   private BarSeries usedSeries;
   private BarSeries freeSeries;

   public SignalsPerSignalSizeChart()
   {
      super("Signals / Signal Size",
            "The number of allocated and freed signals per signal size.",
   ...
   protected Chart createChart()
   {
      ...
      sdY.getSeriesPalette().getEntries().add(ColorDefinitionImpl.create(180, 114, 116));
   ...
   public void update(Object input)
   {
      ...
      for (int i = 0; i < signals.length; i++)
      {
         ...
         key = new Integer(signalSize);

         cd = (ChartData) map.get(key);
         if (cd == null)
         {
            cd = new ChartData(signalSize);
            if (freed)
            {
               cd.setNumFree(1);
            }
   ...
   protected void setInput(Object[] input)
   {
      ...
      for (int i = 0; i < length; i++)
      {
         ...
         xLabels[i] = truncateString(Integer.toString(cd.getSignalSize()));
   ...
   static class ChartDataComparator extends AbstractChartDataComparator
   {
      public int compare(Object o1, Object o2)
      {
         ...
         {
            return compareUnsignedInts(cd1.getSignalSize(), cd2.getSignalSize());
         }
      }
   }
}
