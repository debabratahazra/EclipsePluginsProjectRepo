...
/**
 * This class represents an OSE user event action point. A user event action
 * point defines what kind of action should be taken when a user event occurs.
 *
 * @see com.ose.system.EventActionPoint
 * @see com.ose.system.UserEvent
 */
public final class UserEventActionPoint extends EventActionPoint
{
   ...
   /**
    * Create a new user event action point object.
    */
   public UserEventActionPoint()
   {
      super();
    ...
    * match as user event reporter processes.
    ...
    * @param eventDataIncluded  For ACTION_NOTIFY, ACTION_INTERCEPT, and
    ...
    * to set.
    * @param numberRangeInverted  true if the user event number range specified
    * with numberMin and numberMax is inverted, false otherwise.
    * @param numberMin  the minimal user event number (inclusive)
    * @param numberMax  the maximal user event number (exclusive).
    ...
    */
   public UserEventActionPoint(int state,
                               int fromScopeType,
                               int fromScopeId,
                               ...
                               long filterDataMax)
   {
      super(state,
            ...
            not,
            action,
            createParameter(action, fileAndLineIncluded, eventDataIncluded, newState),
            numIgnore,
            numRemove);
    ...
    */
   UserEventActionPoint(int state,
                        int fromType,
                        int fromId,
                        int toType,
                        int toId,
                        ...
                        boolean bigEndian)
   {
      ...
      if (tags != null)
      {
         ...
         for (int i = 0; i < tags.length; i++)
         {
            ...
            else if (type != Monitor.MONITOR_USER_FILTER_TAG_NOT)
            {
            ...
            try
            {
               switch (type)
               {
                  case Monitor.MONITOR_USER_FILTER_TAG_NOT:
                     break;
                  case Monitor.MONITOR_USER_FILTER_TAG_NUMBER:
            ...
            notTag = (type == Monitor.MONITOR_USER_FILTER_TAG_NOT);
         }
      }
    ...
    * @param eventDataIncluded  For ACTION_NOTIFY, ACTION_INTERCEPT, and
    * ACTION_TRACE actions, determine whether event data will be included in
    * user events or not.
    * @param newState  For ACTION_SET_STATE actions, this is the new event state
    * to set.
   ...
   /**
    * Determine whether the user event number range is inverted or not.
    *
    * @return true if the user event number range is inverted, false otherwise.
    */
   ...
   /**
    * Set whether the user event number range is inverted or not.
    *
    * @param numberRangeInverted  true if the user event number range is
   ...
   /**
    * Return the minimal user event number (inclusive).
    * Only user events with event numbers within the getNumberMin() and
   ...
   /**
    * Set the minimal user event number (inclusive).
    * Only user events with event numbers within the getNumberMin() and
    * getNumberMax() range are reported.
    *
    * @param numberMin  the minimal user event number (inclusive).
   ...
   /**
    * Return the maximal user event number (exclusive).
    * Only user events with event numbers within the getNumberMin() and
    * getNumberMax() range are reported.
    *
    * @return the maximal user event number (exclusive).
   ...
   /**
    * Set the maximal user event number (exclusive).
    * Only user events with event numbers within the getNumberMin() and
    * getNumberMax() range are reported.
    *
    * @param numberMax  the maximal user event number (exclusive).
   ...
   /**
    * Return an array of the OSE monitor protocol tags for this user event
    * action point.
    *
    *@param bigEndian      the target byte order.
    *
   ...
   MonitorTag[] getTags(boolean bigEndian)
   {
      ...
      {
         tags.add(new MonitorTag(Monitor.MONITOR_USER_FILTER_TAG_NOT));
      }
      tags.add(new MonitorTag(Monitor.MONITOR_USER_FILTER_TAG_NUMBER,
   ...
   /**
    * Determine whether event data will be included in user events or not.
    * <p>
    * Only applicable if getAction() returns ACTION_NOTIFY, ACTION_INTERCEPT, or
    * ACTION_TRACE.
    *
    * @return true if event data will be included in user events, false
    * otherwise.
    */
   public boolean isEventDataIncluded()
   {
   ...
   /**
    * Set whether event data will be included in user events or not.
    * <p>
    * Only applicable if getAction() returns ACTION_NOTIFY, ACTION_INTERCEPT, or
    * ACTION_TRACE.
    *
    * @param eventDataIncluded  true if signal data will be included in send
    * events, false otherwise.
    */
   public void setEventDataIncluded(boolean eventDataIncluded)
   {
      int param;
   ...
   /**
    * Return a string representation of this user event action point object.
    *
    * @return a string representation of this user event action point object.
   ...
   public String toString()
   {
      return (super.toString() +
              ", User event number range inverted: " + numberRangeInverted +
              ", Min user event number: " + numberMin +
              ", Max user event number: " + numberMax +
