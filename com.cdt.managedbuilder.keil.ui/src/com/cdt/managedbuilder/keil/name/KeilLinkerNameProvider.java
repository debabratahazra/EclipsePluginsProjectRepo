...
package com.cdt.managedbuilder.keil.name;
...
//
public class KeilLinkerNameProvider implements IManagedOutputNameProvider {
	...
	public IPath[] getOutputNames(ITool tool, IPath[] primaryInputNames) {
		...
		if (hasAncestor(tool, "com.cdt.managedbuild.tool.keil.c.linker")) {	//$NON-NLS-1$
		...
		if (hasAncestor(tool, "com.cdt.managedbuild.tool.keil.cpp.linker")) {	//$NON-NLS-1$
		...
		if (isSO && soName != null && soName.length() > 0) {
			...
			if (toolParent instanceof IConfiguration)
				...
				throw new AssertionError(
						ManagedMakeMessages.getResourceString("KeilLinkerNameProvider.0")); //$NON-NLS-1$
			}
